...
internal static class Program
{
    ...
    private static async Task Main(string[] args)
    {
        ...
        builder.Services.AddSingleton<EncryptedXmlCache>(_ => new EncryptedXmlCache());
        builder.Services.AddSingleton<PortalNfeFallbackLauncher>();
        builder.Services.AddSingleton<FiscalCooldownStore>(sp =>
            new FiscalCooldownStore(sp.GetRequiredService<SharedQueuePaths>()));
        builder.Services.AddSingleton<FiscalOperationGate>(sp =>
            new FiscalOperationGate(sp.GetRequiredService<SharedQueuePaths>()));
        ...
        var sharedPaths = app.Services.GetRequiredService<SharedQueuePaths>();
        TryInitializeSharedFolder(sharedPaths);
        ...
        {
            var currentCertificate = await certificates.GetCurrentAsync(cancellationToken);
            return Results.Ok(new
            {
                ...
                shareAvailable = paths.ValidateForClient(),
                portalFallbackAvailable = currentCertificate is not null,
                sharedFolder = paths.Root,
        ...
        Application.Run(new TrayApplicationContext(sharedPaths));
        await app.StopAsync();
    }

    private static void TryInitializeSharedFolder(SharedQueuePaths paths)
    {
        try
        {
        ...
        {
            // O app local continua disponível para configuração; o gate fiscal bloqueia consultas até a pasta voltar.
        }
    }
